...
AppName="U52.Eliminate_the_Apache_Directory_Listing"
Result_Fail=0
Result_OK=1
Result_Unknown=2
...
do
  ...
  then
    ...
    then
      ...
      apache_check=`cat "${HTTPD_CONF}" | awk -F'#' '{print$1}' | egrep -i "Options" | grep -i "Indexes" | wc -l`
    fi
    ...
    then
      ...
      echo "# httpd.conf grep Indexes"  >> "${LOGGING_FILE}" 2>&1
      cat  ${HTTPD_CONF} | grep -i "Indexes" >> "${LOGGING_FILE}" 2>&1
    else
      if [ "${score}" -eq 0 ]
      then
