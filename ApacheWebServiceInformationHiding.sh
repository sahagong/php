...
AppName="U70.Apache_Web_Service_Information_Hiding"
Result_Fail=0
Result_OK=1
Result_Unknown=2
...
score=0
icheck=0
search_title="ServerTokens"
search_value="Prod"

echo "# Apache Web Service Information Hiding" >> "${LOGGING_FILE}" 2>&1
...
do
  ...
  then
    ...
    then
      ...
      apache_check=`cat "${HTTPD_CONF}" | awk -F'#' '{print$1}' | egrep -i "${search_title}" | grep -i "${search_value}" | wc -l`
    fi
    ...
    if [ "${apache_check}" -eq 1 ]
    then
      echo "RESULT : OK" >> "${LOGGING_FILE}" 2>&1
      echo " " >> "${LOGGING_FILE}" 2>&1
      echo "- httpd.conf : ${HTTPD_CONF} " >> "${LOGGING_FILE}" 2>&1
      echo "# httpd.conf grep ${search_title}"  >> "${LOGGING_FILE}" 2>&1
      cat  ${HTTPD_CONF} | grep -i "${search_title}" >> "${LOGGING_FILE}" 2>&1
    else
      if [ "${score}" -eq 0 ]
      then
